...
export default async function page({
  ...
  const {products, filters, total, max, min} = JSON.parse(await getProducts(category, searchParams));

  return (
    <main className="mt-20 loading-wrapper">
      {products.length > 0 ? (
        <div className="px-5 mb-5 max-sm:py-10 flex gap-3 relative">
          <FilterSlide category={category} searchParams={searchParams} filters={filters} max={max} min={min} />
          <div className="w-full shrink loading-section">
            <div>
              <div className="flex justify-between items-center gap-3 flex-wrap my-5 max-w-[1400px]">
                <div>
                  <FilterSort searchParams={searchParams} category={category} />
                </div>
                <div>
                  <FilterPage
